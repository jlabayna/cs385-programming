...
void solve(State *s, const int aTarget, const int bTarget, const int cTarget, const int a_cap, const int b_cap,
	...
	const int rows = a_cap + 1;
	const int cols = b_cap + 1;
	...
	while (!q->empty()) {
		...
		// I forgot what this delete does...
		// OH! There are possibly duplicate states still created.
		// If somehow, you can get to a state from 2+ others.
		// Still tho. fixing this is marginal perf gain....
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			// Is this if statement even faster?
